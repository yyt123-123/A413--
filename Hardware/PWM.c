...
//	GPIO_PinRemapConfig(GPIO_PartialRemap1_TIM2, ENABLE);//开启重映射功能
//	GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable, ENABLE);//将JTAG复用解除掉，将A0重映射到A15
	...
	TIM_TimeBaseInitStructure.TIM_Period = 100 - 1;//ARR（周期）:在10K频率下计10000个数就是1s，ARR：当定时器的计数值从 0 开始计数，达到这个设定的自动重装载值（10000 - 1 = 9999）后，定时器会产生一个更新事件
	TIM_TimeBaseInitStructure.TIM_Prescaler = 36 - 1;//PSC:对72M进行7200的分频得到10K的计数频率，PSC：计数器每秒钟会被时钟信号驱动计数 10000 次。
